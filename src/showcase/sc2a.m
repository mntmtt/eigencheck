fprintf(" ----- Example 2a: maybe not everything... -----\n\n");
fprintf("What about high multiplicities?\n");
fprintf("High multiplicities are a problem for the conditioning of the determinant function.\n\n");
...
[eig_results(1), alg_results(1), flags(1)]  = algebraic_multiplicity(M, random_error(-50, 0.10), 1e-8, 5, 8);
geo_results(1) = geometric_multiplicity(M, eig_results(1), 1e-8);
[eig_results(2), alg_results(2), flags(2)] = algebraic_multiplicity(M, random_error(-30, 0.10), 1e-8, 5, 8);
geo_results(2) = geometric_multiplicity(M, eig_results(2), 1e-8);
[eig_results(3), alg_results(3), flags(3)] = algebraic_multiplicity(M, random_error(-10, 0.10), 1e-8, 5, 8);
geo_results(3) = geometric_multiplicity(M, eig_results(3), 1e-8);
[eig_results(4), alg_results(4), flags(4)]  = algebraic_multiplicity(M, random_error(10, 0.10), 1e-8, 5, 8);
geo_results(4) = geometric_multiplicity(M, eig_results(4), 1e-8);
[eig_results(5), alg_results(5), flags(5)] = algebraic_multiplicity(M, random_error(30, 0.10), 1e-8, 5, 8);
geo_results(5) = geometric_multiplicity(M, eig_results(5), 1e-8);
[eig_results(6), alg_results(6), flags(6)] = algebraic_multiplicity(M, random_error(50, 0.10), 1e-8, 5, 8);
